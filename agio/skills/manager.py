...
class SkillManager:
    ...
    def _resolve_skill_dirs(self) -> list[Path]:
        ...
        Supports environment variable AGIO_SKILLS_DIR (comma-separated)
        in addition to configured directories.
        ...
        env_dirs = os.getenv("AGIO_SKILLS_DIR")
        if env_dirs:
            for env_dir in env_dirs.split(","):
