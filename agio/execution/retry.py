...
async def retry_from_sequence(
    ...
    logger.info(
        "retry_steps_deleted", session_id=session_id, sequence=sequence, deleted_count=deleted_count
    )
    ...
    if not last_step:
        logger.warning("retry_no_steps_remaining", session_id=session_id)
    ...
    if last_step.is_user_step():
        ...
        if last_step.has_tool_calls():
            ...
            logger.warning(
                "retry_from_final_assistant", session_id=session_id, step_id=last_step.id
            )
...
async def retry_last_response(
    ...
    async for event in retry_from_sequence(session_id, last_assistant_seq, repository, runner):
        yield event
