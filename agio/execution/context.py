...
async def build_context_from_steps(
    session_id: str, repository: AgentRunRepository, system_prompt: str | None = None
) -> list[dict]:
    """
...
async def build_context_from_sequence_range(
    ...
    """
    logger.debug(
        "building_context_range", session_id=session_id, start_seq=start_seq, end_seq=end_seq
    )

    # Query with sequence range
    steps = await repository.get_steps(session_id, start_seq=start_seq, end_seq=end_seq)

    logger.debug("context_range_steps_loaded", session_id=session_id, count=len(steps))

    # Convert to messages using StepAdapter
    messages = StepAdapter.steps_to_messages(steps)
    ...
    for i, msg in enumerate(messages):
        ...
        if role == "tool":
            ...
            if msg["tool_call_id"] not in tool_call_ids:
                logger.warning(
                    "tool_call_id_not_found", message_index=i, tool_call_id=msg["tool_call_id"]
                )

    return True


async def get_context_summary(session_id: str, repository: AgentRunRepository) -> dict:
    """
