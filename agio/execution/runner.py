...
class StepRunner:
    ...
    async def run_stream(
        ...
        executor = StepExecutor(
            ...
            config=ExecutionConfig(max_steps=self.config.max_steps),
        )
        ...
        try:
            ...
            run.status = RunStatus.FAILED  # 或者定义 RunStatus.CANCELLED
            run.metrics.end_time = time.time()
            run.metrics.duration = run.metrics.end_time - run.metrics.start_time
